...
from ..items import BancadiimolaItem
from itemloaders.processors import TakeFirst


class BancadiimolaSpider(scrapy.Spider):
	name = 'bancadiimola'
	start_urls = ['https://www.bancadiimola.it/ita/News']
	...
	def parse_post(self, response):
		...
		description = response.xpath(
			'//article//text()[normalize-space() and not(ancestor::h2 | ancestor::p[@class="date"] | ancestor::a)]').getall()
		description = [p.strip() for p in description]
		description = ' '.join(description).strip()
		date = response.xpath('//article/p[@class="date"]/text()').get()

		item = ItemLoader(item=BancadiimolaItem(), response=response)
